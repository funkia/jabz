...
export abstract class AbstractApplicative<A> extends AbstractFunctor<A>
  implements Applicative<A> {
  abstract of<B>(b: B): Applicative<B>;
  ap<B>(f: Applicative<(a: A) => B>): Applicative<B> {
    return this.lift<(a: A) => B, A, B>(apply, f, this);
  }
  lift<T1, R>(f: (t: T1) => R, m: Applicative<T1>): Applicative<R>;
  lift<T1, T2, R>(
    f: (t: T1, u: T2) => R,
...
export function applicative(constructor: Function): void {
  ...
  if (!("ap" in prototype) && !("lift" in prototype)) {
    throw new TypeError(
      "Can't derive applicative. Either `lift` or `ap` method must be defined."
    );
  }
...
export function lift<T1, T2, R>(
  f: (t: T1, u: T2) => R,
  m1: Applicative<T1>,
  m2: Applicative<T2>
): any;
export function lift<T1, T2, T3, R>(
  f: (t1: T1, t2: T2, t3: T3) => R,
  m1: Applicative<T1>,
  m2: Applicative<T2>,
  m3: Applicative<T3>
...
export function lift<A, T1, T2, R>(
  f: (t: T1, u: T2) => R,
  m1: Either<A, T1>,
  m2: Either<A, T2>
): Either<A, R>;
export function lift<A, T1, T2, T3, R>(
  f: (t1: T1, t2: T2, t3: T3) => R,
  m1: Either<A, T1>,
  m2: Either<A, T2>,
  m3: Either<A, T3>
