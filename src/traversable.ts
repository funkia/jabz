...
export abstract class AbstractTraversable<A> extends AbstractFoldable<A>
  implements Traversable<A> {
  map<B>(f: (a: A) => B): Traversable<B> {
    return (<any>this.traverse(Identity, (a: A) =>
      Identity.of(f(a))
    )).extract();
  }
...
export function traversable(constructor: Function): void {
  ...
  if (!("map" in p && "sequence" in p) && !("traverse" in p)) {
    throw new TypeError(
      "Can't derive traversable. Either `traverse` or `map` and `sequence` must be defined."
    );
  }
...
export function mapAccumR<A, B, C>(
  ...
  return (<any>t.traverse(
    AnApplicative,
    (a: A) => new AnApplicative((c: C) => f(c, a))
  )).run(init);
}
