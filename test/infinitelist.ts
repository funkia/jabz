...
describe("InfiniteList", () => {
  describe("functor", () => {
    it("can map elements", () => {
      assert.deepEqual(take(5, map(n => n * n, naturals)), [0, 1, 4, 9, 16]);
    });
    it("can mapto", () => {
      assert.deepEqual(take(5, mapTo(8, naturals)), [8, 8, 8, 8, 8]);
    });
  });
  describe("foldable", () => {
    ...
    it("has left fold", () => {
      assert.deepEqual(
        shortFoldl(
          (acc, n) => (n > 5 ? left(acc) : right(acc - n)),
          0,
          naturals
        ),
        0 - 1 - 2 - 3 - 4 - 5
  ...
  describe("applicative", () => {
    it("gives repeating list in of", () => {
      assert.deepEqual(take(5, naturals.of(5)), [5, 5, 5, 5, 5]);
    });
    it("ap", () => {
      assert.deepEqual(
    ...
    it("lifts function of two arguments", () => {
      assert.deepEqual(take(5, lift((a, b) => [a, b], naturals, repeat(0))), [
        [0, 0],
        [1, 0],
        [2, 0],
        [3, 0],
